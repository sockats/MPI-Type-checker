...
void st_tree_print(const st_tree *tree, int isScribble)
{
  //int i;
...
/*
    ...
    for (i=0; i<tree->info->nrole; ++i)
      if (tree->info->nrole <= 1){
	printf("%s", tree->info->roles[i]);
      } else if (tree->info->nrole > 1 && i == (tree->info->nrole - 1)) {
	printf("%s", tree->info->roles[i]);
      }
      else {
	printf("%s, ", tree->info->roles[i]);
...
*/
  ...
  if (tree->root != NULL) {
    st_node_print_r(tree->root, 0, isScribble);
  } else {
...
void st_node_print_r(const st_node *node, int indent, int isScribble)
{
  int i;

  st_node_print(node, indent, isScribble);
  for (i=0; i<node->nchild; ++i) {
    st_node_print_r(node->children[i], indent+1, isScribble);
  }
}

//Note: Where was "op" (node->interaction->msgsig.op - label of message) in print a printf,
//it has been replaced with "tag" for the purpose of MPI typechecker
void st_node_print(const st_node *node, int indent, int isScribble)
{
  int i, j;
  ...
  if (node != NULL) {
	...
	if(isScribble == 1)
	{
        ...
        printf(", msgsig: { tag: %s, payload: %s }", node->interaction->msgsig.op, node->interaction->msgsig.payload);
        ...
        if(isScribble == 1)
	{
      ...
      case ST_NODE_BCAST: // ---------- BCAST ----------
        printf("Node { type: MPI_Bcast, root: ");
        printf("%s", node->interaction->from); //this is the rank "root" (which process is doing the broadcast) - MPI addon
     ...
     case ST_NODE_REDUCE: // ---------- REDUCE ----------
        printf("Node { type: MPI_Reduce, root: ");
        printf("%s", node->interaction->to[0]); //this is the rank "root" (which process is doing the reduce) - MPI addon
    ...
    case ST_NODE_ALLREDUCE: // ---------- ALLREDUCE ----------
        printf("Node { type: MPI_Allreduce, from/to: ");
        printf("%s", node->interaction->to[0]); // this will be "-"
	...
	if(isScribble == 1)
	{
		...
		printf("Node { type: MPI_Recv, from: ");
	}
        if (ST_ROLE_PARAMETRISED == node->interaction->from_type) { // ST_ROLE_PARAMETRISED
          printf("%s[%s:", node->interaction->p_from->name, node->interaction->p_from->bindvar);
          for (j=0; j<node->interaction->p_from->idxcount; ++j) {
      ...
      case ST_NODE_IRECV: // ----------- IRECV -----------
        if(isScribble == 1)
	{
		printf("Node { type: Recv, to: ");
	}else {
		printf("Node { type: MPI_Irecv, from: ");
	}
        if (ST_ROLE_PARAMETRISED == node->interaction->from_type) { // ST_ROLE_PARAMETRISED
          printf("%s[%s:", node->interaction->p_from->name, node->interaction->p_from->bindvar);
          for (j=0; j<node->interaction->p_from->idxcount; ++j) {
      ...
      case ST_NODE_CHOICE: // ---------- CHOICE ----------
        printf("Node { type: choice, var depend.: %s } %d children \n", node->choice->at, node->nchild);
        break;
...
//compare trees recursivelt starting from twoo given nodes
int st_node_compare_r(st_node *node, st_node *other)
{
  int identical = 1;
  int i;

  //if nodes to compare are not null
...
//compare message signature
int st_node_compare_msgsig(const st_node_msgsig_t msgsig, const st_node_msgsig_t other)
{
  //compare msgsig.payload. msgsig.op does not matter in MPI since we keep the tags there 
  return (0 == strcmp(msgsig.payload, other.payload));
}

//compare two nodes
int st_node_compare(st_node *node, st_node *other)
{
  int collectiveOper = 0;

  int identical = 1;
  //if both not null
  if (node != NULL && other != NULL) {
    
    //bcast and send matching - on MPI bcast and on scribble send
    if(node->type ==  ST_NODE_BCAST && other->type == ST_NODE_SEND && node->nchild == other->nchild)
    {
 	identical = 1;
	collectiveOper = 1;
    }else if(node->type ==  ST_NODE_SEND && other->type == ST_NODE_BCAST && node->nchild == other->nchild)
    {
	identical = 1;
	collectiveOper = 1;

     //reduce and recv matching - on MPI reduce and on scrivvle recv
    }else if(node->type ==  ST_NODE_REDUCE && other->type == ST_NODE_RECV  && node->nchild == other->nchild)
    {
 	identical = 1;
	collectiveOper = 1;
    }else if(node->type ==  ST_NODE_RECV && other->type == ST_NODE_REDUCE && node->nchild == other->nchild)
    {
	identical = 1;
	collectiveOper = 1;
    }else{
   ...
   //if non of the above collective operators occure then check other cases
   if(collectiveOper == 0)
   {  //compare node types
      identical = (node->type == other->type && node->nchild == other->nchild);
   }

    //if node types are the same
    if (identical) {
      switch (node->type) {
        case ST_NODE_ROOT:
          break;
        ...
        case ST_NODE_BCAST:
          identical &= st_node_compare_msgsig(node->interaction->msgsig, other->interaction->msgsig);
          identical &= (node->interaction->nto == other->interaction->nto);
	  //to_type either ST_NORMAL or ST_PARAMETARISED
          identical &= (node->interaction->to_type == other->interaction->to_type);

          if (identical) {
            int i, j;
            for (i=0; i<node->interaction->nto; ++i) {
              //comparison to to_type has be done above - now depending on the to_type do the following
              if (ST_ROLE_NORMAL == node->interaction->to_type) { 
		if(collectiveOper == 0)
		{
		   //get rank from scribble node because it contains the letter "R" in front
	 	   char *to = (char*) malloc(sizeof(node->interaction->to[i])-1);
		   to = strndup(node->interaction->to[i]+1, strnlen(node->interaction->to[i], 20)-1);

		   //compare if the destination of the message in both nodes are the same
                   identical &= (0 == strcmp(to, other->interaction->to[i]));
		}else{
		   identical &= (0 == strcmp(node->interaction->to[i], other->interaction->to[i]));
		}
              } /*else if (ST_ROLE_PARAMETRISED == node->interaction->from_type) {
        ...
        case ST_NODE_REDUCE:
          ...
          if (identical) {
            int j;
            if (ST_ROLE_NORMAL == node->interaction->from_type) {
              //if not collective operator
	      if(collectiveOper == 0)
              {
	        //get rank from scribble node because it contains the letter "R" in front
	        char *from = (char*) malloc(sizeof(node->interaction->from));
	        from = strndup(node->interaction->from+1, strnlen(node->interaction->from, 20)-1);

                identical &= (0 == strcmp(from, other->interaction->from));
	      }else{
		identical &= (0 == strcmp(node->interaction->from, other->interaction->from));
	      }
            }/* else if (ST_ROLE_PARAMETRISED == node->interaction->from_type) {
          ...
          break;
        case ST_NODE_CHOICE:
	  //we do not need to compare that in MPI - projection has taken care of that
